...
import { MISSIONTWOTEMPTWOCOMPLETEROLEID } from "../../globals";
...
export const ManagerMissionTwoSelectMenuCollector = async (interaction: StringSelectMenuInteraction<CacheType>, client: ClientWithCommands, serverConfigs: GuildDocument) => {
  if(interaction.customId.split('-')[2] === 'part1'){
    if(interaction.guild?.members.cache.get(interaction.user.id)?.roles.cache.has(MISSIONTWOTEMPTWOCOMPLETEROLEID)){
